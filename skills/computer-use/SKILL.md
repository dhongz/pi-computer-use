...
* The harness also exposes direct `computer-use_*` tools. If Sky bootstrap fails (for example, `Sky Computer Use native pipe startup failed`) or a Sky call times out, immediately fall back to the corresponding direct tool instead of retrying repeatedly:
  ...
  * `sky.click` → `computer-use_click`
  * `sky.set_value` → `computer-use_set_value`
  * `sky.type_text` → `computer-use_type_text`
  * `sky.press_key` → `computer-use_press_key`
  * `sky.scroll` → `computer-use_scroll`
  * `sky.select_text` → `computer-use_select_text`
  * `sky.drag` → `computer-use_drag`
  * `sky.perform_secondary_action` → `computer-use_perform_secondary_action`
* Do not use other technologies for UI interaction while a Computer Use transport is working. If both transports are unavailable, an OS-level launcher such as `open -a "Google Chrome" <url>` is acceptable only as a last-resort fallback for an explicitly requested, low-risk navigation. Do not use that fallback for clicks, form entry, uploads, submissions, purchases, or settings changes.
...
Start by getting the state for the app you want to use. When the task names an app, use its display name first (for example, `Google Chrome`), then retry with the bundle identifier from `list_apps()` if the display name fails:
...
For the direct-tool fallback, use `computer-use_get_app_state` with the same display name first. If that fails, call `computer-use_list_apps` and retry with the matching bundle identifier.
...
After performing one or more UI actions, call `get_app_state(...)` (or the direct equivalent) before deciding what to do next. This keeps you in the current UI state and forces you to re-derive fresh `element_index` values from the latest accessibility text instead of reusing stale ones.
